...
const prompt = `Runs JavaScript code to interact with the Privy API API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        PRIVY_APP_ID: requireValue(
          readEnv('PRIVY_APP_ID') ?? client.appID,
          'set PRIVY_APP_ID environment variable or provide appID client option',
        ),
        PRIVY_APP_SECRET: requireValue(
          readEnv('PRIVY_APP_SECRET') ?? client.appSecret,
          'set PRIVY_APP_SECRET environment variable or provide appSecret client option',
        ),
        PRIVY_API_BASE_URL:
          readEnv('PRIVY_API_BASE_URL') ?? readEnv('PRIVY_API_ENVIRONMENT') ?
            undefined
          : client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'privy-api-client',
      code,
      intent,
      client_opts: { environment: (readEnv('PRIVY_API_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@privy-io/node';
